...
// Open 开启一个数据库实例
func Open(dirname, filename string) (*MiniBitcask, error) {
	// 如果数据库目录不存在，则新建一个
...
func (db *MiniBitcask) Merge() error {
	...
	// 新建临时文件
	mergeDBFile, err := NewMergeDBFile(dbDir, db.dbName)
	if err != nil {
		return err
	}
	defer func() {
	...
	// 临时文件变更为新的数据文件
	_ = os.Rename(mergeDBFileName, filepath.Join(dbDir, db.dbName))
